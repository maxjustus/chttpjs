...
describe('ClickHouse Integration Tests', { timeout: 60000 }, () => {
  ...
  describe('Compression methods', () => {
    it('should insert with LZ4 compression', async () => {
      ...
      const data = Array.from({ length: 1000 }, (_, i) => ({ value: `test_${i}` }));
      ...
      // Clean up
      for await (const chunk of execQuery('DROP TABLE test_lz4', sessionId, false, { baseUrl, auth })) {
        // consume stream
      }
    });
    ...
    it('should insert with ZSTD compression', async () => {
      ...
      // Clean up
      for await (const chunk of execQuery('DROP TABLE test_zstd', sessionId, false, { baseUrl, auth })) {
        // consume stream
      }
    });
  });
  ...
  describe('Streaming inserts with generators', () => {
    it('should handle generator that yields batches', async () => {
      ...
      // Clean up
      for await (const chunk of execQuery('DROP TABLE test_generator', sessionId, false, { baseUrl, auth })) {
        // consume stream
      }
    });
    ...
    it('should handle generator that yields single rows', async () => {
      ...
      // Clean up
      for await (const chunk of execQuery('DROP TABLE test_single', sessionId, false, { baseUrl, auth })) {
        // consume stream
      }
    });
  });
  ...
  describe('Streaming queries with compression', () => {
    it('should stream compressed query results', async () => {
      ...
      // Clean up
      for await (const chunk of execQuery('DROP TABLE test_stream', sessionId, false, { baseUrl, auth })) {
        // consume stream
      }
    });
  ...
  describe('Error handling', () => {
    it('should handle invalid queries', async () => {
      try {
        ...
        assert.ok(error.message.includes('UNKNOWN_TABLE') || error.message.includes('doesn\'t exist'));
      }
    });
    ...
    it('should handle insert errors', async () => {
      ...
      try {
        ...
        assert.ok(error.message.includes('TYPE_MISMATCH') || error.message.includes('Cannot parse'));
      }

      // Clean up
      for await (const chunk of execQuery('DROP TABLE test_error', sessionId, false, { baseUrl, auth })) {
        // consume stream
      }
    });
  });
