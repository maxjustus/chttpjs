...
// Generate test data
function generateSimpleData(count: number): { columns: ColumnDef[]; rows: unknown[][] } {
...
function generateMixedData(count: number): { columns: ColumnDef[]; rows: unknown[][] } {
  const columns: ColumnDef[] = [
    { name: "id", type: "UInt32" },
  ...
  const rows: unknown[][] = [];
  for (let i = 0; i < count; i++) {
    rows.push([i, `user_${i}`, `user${i}@example.com`, i % 2 === 0, Math.random() * 100]);
  }
  return { columns, rows };
}

function generateComplexData(count: number): { columns: ColumnDef[]; rows: unknown[][] } {
  const columns: ColumnDef[] = [
    { name: "id", type: "UInt32" },
...
async function main() {
  ...
  console.log(`Benchmarking data access with ${ROWS} rows, ${ITERATIONS} iterations\n`);
  ...
  for (const scenario of scenarios) {
    ...
    const toArrayResult = bench("  toArrayRows() (get loop)", () => toArrayRows(decoded), 50, ITERATIONS);
    const hybridToArray = bench("  hybrid (direct + get)", () => toArrayRowsHybrid(decoded), 50, ITERATIONS);
    console.log(`  ${toArrayResult.name.padEnd(25)} ${toArrayResult.ms.toFixed(3).padStart(8)}ms`);
    console.log(`  ${hybridToArray.name.padEnd(25)} ${hybridToArray.ms.toFixed(3).padStart(8)}ms`);
    console.log(`  Hybrid speedup: ${(toArrayResult.ms / hybridToArray.ms).toFixed(2)}x\n`);

    console.log("asRows() (collected):");
    const asRowsResult = bench("  asRows()", () => collectRows(asRows(decoded)), 50, ITERATIONS);
    console.log(`  ${asRowsResult.name.padEnd(25)} ${asRowsResult.ms.toFixed(3).padStart(8)}ms\n`);

    console.log("Table (Proxy rows):");
    const table = Table.from(decoded);
    const tableResult = bench("  Table iter (access 1)", () => collectRowsTable(table), 50, ITERATIONS);
    const tableFullResult = bench("  Table iter (toObject)", () => collectRowsTableFull(table), 50, ITERATIONS);
    console.log(`  ${tableResult.name.padEnd(25)} ${tableResult.ms.toFixed(3).padStart(8)}ms`);
    console.log(`  ${tableFullResult.name.padEnd(25)} ${tableFullResult.ms.toFixed(3).padStart(8)}ms\n`);
  }
}
