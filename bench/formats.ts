...
async function benchAsync(name: string, fn: () => Promise<void>, warmup = 50, iterations = 100): Promise<{ name: string; ms: number }> {
...
function decodeJsonEachRow<T>(data: Uint8Array): T[] {
  return decoder.decode(data).trim().split("\n").map((line) => JSON.parse(line) as T);
}
...
async function runScenario(scenario: Scenario, iterations: number): Promise<ScenarioResult> {
  ...
  const jsonEnc = bench("JSONEachRow encode", () => encodeJsonEachRow(scenario.jsonData), 50, iterations);
  ...
  // Decoding
  console.log("\nDecoding:");
  const jsonDec = bench("JSONEachRow decode", () => decodeJsonEachRow(jsonEncoded), 50, iterations);
  console.log(formatResult(jsonDec, rows));
  const rbDec = bench("RowBinary decode", () => decodeRowBinary(rbEncoded), 50, iterations);
  console.log(formatResult(rbDec, rows));
  const nativeDec = await benchAsync("Native decode", async () => { await decodeNative(nativeEncoded); }, 50, iterations);
  ...
  const jsonFull = bench("JSONEachRow + LZ4", () => encodeBlock(encodeJsonEachRow(scenario.jsonData), Method.LZ4), 50, iterations);
  ...
  return {
    ...
    size: { json: jsonEncoded.length, rb: rbEncoded.length, native: nativeEncoded.length },
    compressed: { json: jsonComp.length, rb: rbComp.length, native: nativeComp.length },
  };
}

// --- Data generators ---

function generateSimpleData(count: number): { json: Record<string, unknown>[]; rows: unknown[][]; columns: ColumnDef[] } {
  ...
  for (let i = 0; i < count; i++) {
    ...
    json.push({ id: i, name: `user_${i}`, email: `user${i}@example.com`, active: i % 2 === 0, score: Math.random() * 100, created_at });
    rows.push([i, `user_${i}`, `user${i}@example.com`, i % 2 === 0, Math.random() * 100, created_at]);
  }
  return { json, rows, columns };
}

function generateEscapeData(count: number): { json: Record<string, unknown>[]; rows: unknown[][]; columns: ColumnDef[] } {
  const columns: ColumnDef[] = [
    { name: "id", type: "UInt32" },
    { name: "name", type: "String" },
  ...
  const rows: unknown[][] = [];
  for (let i = 0; i < count; i++) {
    json.push({ id: i, name: `user "test" ${i}`, desc: `Line1\nLine2\tTabbed`, path: `C:\\Users\\test\\file${i}.txt` });
    rows.push([i, `user "test" ${i}`, `Line1\nLine2\tTabbed`, `C:\\Users\\test\\file${i}.txt`]);
  }
  return { json, rows, columns };
}

function generateComplexData(count: number): { json: Record<string, unknown>[]; rows: unknown[][]; columns: ColumnDef[] } {
  const columns: ColumnDef[] = [
    { name: "id", type: "UInt32" },
...
function generateComplexTypedData(count: number): { json: Record<string, unknown>[]; rows: unknown[][]; columns: ColumnDef[] } {
  const columns: ColumnDef[] = [
    { name: "id", type: "UInt32" },
    { name: "tags", type: "Array(String)" },
    { name: "scores", type: "Array(Float64)" },
...
async function main() {
  ...
  const scenarios: Scenario[] = [
    { name: "Simple Data", description: "6 columns: int, 2 strings, bool, float, datetime", columns: simple.columns, jsonData: simple.json, rowsArray: simple.rows },
    { name: "Escape Data", description: "strings with quotes, newlines, backslashes", columns: escape.columns, jsonData: escape.json, rowsArray: escape.rows },
    { name: "Complex Data", description: "arrays, nullable", columns: complex.columns, jsonData: complex.json, rowsArray: complex.rows },
    { name: "Complex Data (Typed)", description: "arrays as TypedArrays", columns: complexTyped.columns, jsonData: complexTyped.json, rowsArray: complexTyped.rows },
  ];
  ...
  const fmtSpeed = (json: number, rb: number, native: number) => `RB ${(json / rb).toFixed(2)}x, Native ${(json / native).toFixed(2)}x`;
  const fmtSize = (json: number, rb: number, native: number) => `RB ${(json / rb).toFixed(2)}x, Native ${(json / native).toFixed(2)}x smaller`;

  for (const r of results) {
    console.log(`${r.name}:`);
    console.log(`  Encode: ${fmtSpeed(r.encode.json, r.encode.rb, r.encode.native)}`);
    console.log(`  Decode: ${fmtSpeed(r.decode.json, r.decode.rb, r.decode.native)}`);
    console.log(`  Size:   ${fmtSize(r.size.json, r.size.rb, r.size.native)}`);
    console.log(`  +LZ4:   ${fmtSize(r.compressed.json, r.compressed.rb, r.compressed.native)}`);
    console.log("");
  }
  ...
  const syncDec = bench("Sync decode", () => decodeRowBinary(simpleRbEncoded), 50, ITERATIONS);
  console.log(formatResult(syncDec, ROWS));

  const stream1 = await benchAsync("Stream decode (1 chunk)", async () => { await collectRowBinary(chunkedStream(simpleRbEncoded, simpleRbEncoded.length)); }, 50, ITERATIONS);
  console.log(formatResult(stream1, ROWS));

  const stream64k = await benchAsync("Stream decode (64KB chunks)", async () => { await collectRowBinary(chunkedStream(simpleRbEncoded, 64 * 1024)); }, 50, ITERATIONS);
  console.log(formatResult(stream64k, ROWS));

  const stream4k = await benchAsync("Stream decode (4KB chunks)", async () => { await collectRowBinary(chunkedStream(simpleRbEncoded, 4 * 1024)); }, 50, ITERATIONS);
  console.log(formatResult(stream4k, ROWS));

  console.log("\nEncoding (sync vs streaming):");
  const syncEnc = bench("Sync encode", () => encodeRowBinary(simple.columns, simple.rows), 50, ITERATIONS);
  console.log(formatResult(syncEnc, ROWS));

  const streamEnc = await benchAsync("Stream encode", async () => { await collectChunks(streamEncodeRowBinary(simple.columns, simple.rows)); }, 50, ITERATIONS);
  console.log(formatResult(streamEnc, ROWS));

  console.log("\nStreaming overhead:");
  console.log(`  Decode (1 chunk): ${((stream1.ms / syncDec.ms - 1) * 100).toFixed(1)}% overhead`);
